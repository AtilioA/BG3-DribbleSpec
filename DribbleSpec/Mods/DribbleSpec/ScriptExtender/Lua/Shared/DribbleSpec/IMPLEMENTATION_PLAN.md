...
- Domain matchers: `toBeUuid`, `toBeEntity`, `toHaveComponent`.
...
- Phase 7 is complete: `EntityRef` lazy re-resolution, entity domain matchers (`toBeUuid`, `toBeEntity`, `toHaveComponent`), and optional `toEqual(..., { volatilePreset = "entity" })` filtering are implemented and covered by self-tests.
